...
int main() {
    ...
    GlfwWindow canvas("MotorController (Metal)", {{"aa", 6}, {"clientAPI", "Metal"}});
    ...
    // 电机参数
    double resistance = 2.0;   // 欧姆
    double inertia = 0.1;      // kg * m^2，转动惯量
    double damping = 0.05;     // N * m / rad / s，阻尼系数
    float targetPosition = 120;// 角度

    // 创建直流电机对象
    DCMotor motor(resistance, inertia, damping);

    // 创建 PID 控制器对象
    PIDRegulator controller(1.0, 0.1, 0.2);// 可按需调整 PID 增益
