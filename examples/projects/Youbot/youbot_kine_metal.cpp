...
namespace {
    struct YoubotUI: ImguiContext {
        ...
        explicit YoubotUI(const GlfwWindow& canvas, Renderer& renderer, Kine& kine)
...
int main() {
    ...
    GlfwWindow canvas{GlfwWindow::Parameters().title("Youbot-kine (Metal)").size({1280, 720}).antialiasing(8).clientAPI(GlfwWindow::ClientAPI::Metal)};
