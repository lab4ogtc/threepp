...
struct CraneUI: ImguiContext {
    ...
    explicit CraneUI(const GlfwWindow& canvas, Renderer& renderer, Kine& kine)
...
int main() {
    ...
    GlfwWindow canvas{"Crane3R (Metal)", {{"size", WindowSize{1280, 720}}, {"aa", 8}, {"clientAPI", "Metal"}}};
    ...
    auto future = std::async(std::launch::async, [&] {
        auto loadedCrane = Crane3R::create();
        loadedCrane->setTargetValues(asAngles(kine.meanAngles(), Angle::Repr::DEG));
        loadedCrane->traverseType<Mesh>([](Mesh& m) {
            m.castShadow = true;
        });

        tm.invokeLater([&, loadedCrane] {
            crane = loadedCrane;
            hud.remove(handle);
            scene->add(loadedCrane);
