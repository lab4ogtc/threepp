...
int main() {
    ...
    GlfwWindow canvas("Sphero simulator (Metal)", {{"aa", 8}, {"clientAPI", "Metal"}});
    ...
    opts.anisotropy = 16;
    auto renderTarget = RenderTarget::create(textureSize, textureSize, opts);
    ...
    canvas.animate([&] {
        ...
        const auto previewY = size.height() > textureSize ? size.height() - textureSize : 0;
        renderer->setViewport(0, previewY, textureSize, textureSize);
