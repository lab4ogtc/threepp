...
- `dev-mswin` preset 必须启用 `THREEPP_WITH_VULKAN=ON`、`THREEPP_BUILD_EXAMPLES=ON`、`THREEPP_BUILD_EXAMPLE_PROJECTS=ON`、`THREEPP_BUILD_TESTS=ON`，确保 Vulkan examples 与 `VulkanGolden_test` 同一套配置下验证。
...
- `VulkanGolden_test` 是图像回归基线。能构建时必须运行 `ctest -R VulkanGolden_test --output-on-failure`；有 RT GPU 时应通过，缺 RT GPU 时允许按 skip code 42 跳过。若测试因工具链或依赖配置无法构建，记录为环境阻塞，不能宣称阶段通过。
