...
int main() {
    Canvas canvas(Canvas::Parameters()
                          .title("VulkanSkinnedMeshRuntime_test")
                          .size({128, 128})
                          .vsync(false));
    ...
    try {
        ...
        geometry->setAttribute("skinIndex", FloatBufferAttribute::create({
                0.f, 0.f, 0.f, 0.f,
                0.f, 0.f, 0.f, 0.f,
                0.f, 0.f, 0.f, 0.f,
                0.f, 0.f, 0.f, 0.f,
        }, 4));
        geometry->setAttribute("skinWeight", FloatBufferAttribute::create({
                1.f, 0.f, 0.f, 0.f,
                1.f, 0.f, 0.f, 0.f,
                1.f, 0.f, 0.f, 0.f,
                1.f, 0.f, 0.f, 0.f,
        ...
        canvas.animate([&] {
            ...
            std::printf("[phase4] SkinnedMesh rightRed=%d leftRed=%d bytes=%zu -> %s\n",
                        rightRed, leftRed, framebuffer.size(), pass ? "PASS" : "FAIL");
            std::exit(pass ? 0 : 1);
        });
    } catch (const std::exception& e) {
        std::printf("[phase4] SkinnedMesh threw: %s\n", e.what());
        return 1;
    }
